...
class PhraseMultiheadAttention(MultiheadAttention):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.ngram_sizes is not None: 
            ...
            ngram_size_info = dict([("{}_gram_features".format(gram_size), NgramLSTM(gram_size, 2*self.head_dim))
        ...
        if self.n_gram_features is not None:
            ...
            for gram_size, count_h_using in self.n_gram_features_count.items():
                if gram_size == 0:
                    ...
                    continue
                ngram_features_extractor = self.n_gram_features["{}_gram_features".format(gram_size)]
                _xx = torch.cat([ _q0.view(bsz, self.num_heads, -1, self.head_dim).reshape(-1, src_len, self.head_dim),
                                  _k0.view(bsz, self.num_heads, -1, self.head_dim).reshape(-1, tgt_len, self.head_dim),
                                  ], dim=-1).reshape(-1, src_len, 2*self.head_dim)
                _yy = ngram_features_extractor(_xx).reshape(-1, src_len, 2, self.head_dim)  # 2, bsz*num_heads, src_len,  head_dim
                _q = _yy.index_select(2, torch.LongTensor([0]).to(q.device)).squeeze(dim=2)
                _k = _yy.index_select(2, torch.LongTensor([1]).to(q.device)).squeeze(dim=2)
