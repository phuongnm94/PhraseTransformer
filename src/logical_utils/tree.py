...
class STree(object):
    ...
    def _set_by_token_list(self, _tk_list):
        ...
        while i < len(tk_list):
            if tk_list[i] == '(':
                ...
                if (i < 2) or not (
                        (tk_list[i - 2] == 'lambda') and tk_list[i - 1].startswith('$') and len(tk_list[i]) == 1):
    ...
    def apply_bpe(self, p):
        ...
        if len(self.children) <= 1:
            ...
            new_list = []
            for c in self.children:
                if isinstance(c, type(self)) and len(c.children) == 2 and isinstance(c.children[1],
                                                                                     six.string_types) and p.is_match(
            ...
            if len(c_list) >= 2 and is_ordered:
                ...
                i = 0
                while i < len(c_list):
                    if (i + 1 < len(c_list)) and isinstance(c_list[i], six.string_types) and isinstance(c_list[i + 1],
                                                                                                        six.string_types) and p.is_match(
                        c_list[i], c_list[i + 1]):
                        new_list.append(str(p))
                        i += 2
                    ...
                    if len(c_list) >= 2:
                        for i in range(len(c_list) - 1):
                            for j in range(i + 1, len(c_list)):
                                if isinstance(c_list[i], six.string_types) and isinstance(c_list[j],
                                                                                          six.string_types) and p.is_match(
                                    c_list[i], c_list[j]):
                                    found = (i, j)
                                    break
...
def is_tree_eq(t1, t2, not_layout=False):
    try:
        ...
        if isinstance(t2, STree):
            ...
            t2 = STree(t2)
        return str(norm_tree_var(t1.norm(not_layout=not_layout))).lower() == str(
            norm_tree_var(t2.norm(not_layout=not_layout))).lower()
    except Exception:
        return False
